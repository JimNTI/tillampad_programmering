...
void loop() {
  ...
  while (menu == 0) {
    readButton();
    oledWriteDate (0, 10 ,("Date: " + String(now.day()) + "." + String(now.month()) + "." + String(now.year())));
      if (pos >= 180) {
    pos = pos - 180;
    tone(soundPin, 500);
  }
  ...
  while (menu == 1) {
    readButton();
    oledWriteTime (0, 10 ,("Time: " + String(now.hour()) + ":" + String(now.minute()) + ":" + String(now.second())));
      if (pos >= 180) {
    pos = pos - 180;
    tone(soundPin, 500);
  }
  ...
  while (menu == 2) {
    readButton();
    oledWriteTemp (0, 10 , ("Temp: " + String(getTemp())));
      if (pos >= 180) {
    pos = pos - 180;
    tone(soundPin, 500);
  }
...
// reads temperature and converts it to Celcius
  ...
  void oledWriteDate(int x, int y, String date) {
  ...
  void oledWriteTime(int x, int y, String time){
    u8g.firstPage();
    do {
  ...
  void oledWriteTemp(int x, int y, String temp_display){
    u8g.firstPage();
    do {

      u8g.drawStr(x, y, temp_display.c_str());
